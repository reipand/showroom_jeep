...
```php
// Light theme (default)
generateRecaptchaHTML('light', 'normal');

// Dark theme
generateRecaptchaHTML('dark', 'normal');

// Compact size
generateRecaptchaHTML('light', 'compact');
```
...
### `generateRecaptchaHTML($theme, $size)`
- Generates HTML untuk reCAPTCHA widget
- Parameters: theme ('light'/'dark'), size ('normal'/'compact')
